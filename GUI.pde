...
void keyPressed()  // executed each time a key is pressed: sets the Boolean "keyPressed" till it is released   
    ...
    if(key=='C') 
      {
      MyText=getClipboard(); StartClip=P(Start); EndClip=P(End);
      println("  StartClip = P(" + floor(StartClip.x) + "," + floor(StartClip.y) + ");"); 
      println("  EndClip = P("   + floor(EndClip.x)   + "," + floor(EndClip.y)   + ");"); 
      } // text from clipboard written where mouse was when 'C' was pressed
    //if(key=='D') ;  
    //if(key=='E') ;
    //if(key=='F') ;
    //if(key=='G') ; 
    ...
    if(key=='T') {currentFrame=0; animate=false; currentTime=0; }
    //if(key=='U') ;
    //if(key=='V') ;
...
String menu1="?:hlp PIX #:jpg @:pdf $:tif ANI a:toggle t:time T:reset ^:ease GIF -:jpg =:tif W:warp C:clip";
